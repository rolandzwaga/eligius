...
const toOperationDataType = (sourceFile: SourceFile) => (operationData: TypeAliasDeclaration|InterfaceDeclaration) => {
    ...
    if (args.length) {
        ...
        const imports = args.reduce<(ReturnType<ReturnType<typeof toImport>>)[]>((acc, item) => {
...
const createSourceFile = (declaration: VariableDeclaration) => (operationData: TypeAliasDeclaration|InterfaceDeclaration|undefined) => {
    ...
    if (importName) {
        outputSourceFile.addImportDeclaration({ moduleSpecifier: `../${fileName}.ts` })
...
const getDependentPropertyNames = (operationData: TypeAliasDeclaration|InterfaceDeclaration|undefined) => {
    if (!operationData) {
        return '';
    }
    const properties = (operationData.isKind(SyntaxKind.InterfaceDeclaration)) ? operationData.getProperties() : operationData.getDescendantsOfKind(SyntaxKind.TypeLiteral)[0]?.getProperties();
    if (properties) {
        const dependentProps = getPropertiesWithTags(properties)('dependency');
        return dependentProps.map(x => `'${x.getName()}'`).join(', ');
    } else {
        throw new Error(`Could not find properties on statement ${operationData.getName()} of kind: ${operationData.getKindName()}`);
    }
};

const getInputProperties = (operationData: TypeAliasDeclaration|InterfaceDeclaration|undefined) => {
    if (!operationData) {
        return '';
    }
    const properties = (operationData.isKind(SyntaxKind.InterfaceDeclaration)) ? operationData.getProperties() : operationData.getDescendantsOfKind(SyntaxKind.TypeLiteral)[0]?.getProperties();
    const inputProps = getPropertiesWithoutTags(properties)('dependency', 'output');

    return inputProps.map(toProp).join(',\n');
};

const getOutputProperties = (operationData: TypeAliasDeclaration|InterfaceDeclaration|undefined) => {
    if (!operationData) {
        return '';
    }
    const properties = (operationData.isKind(SyntaxKind.InterfaceDeclaration)) ? operationData.getProperties() : operationData.getDescendantsOfKind(SyntaxKind.TypeLiteral)[0]?.getProperties();
...
const getTypeFromProperty = (property: PropertySignature) => {
    ...
    if (type.isTypeParameter()) {
        const found = property.getParent().getType().getTypeArguments().find(a => a.getText() === type.getSymbol()?.getName());
        if (!found) {
            throw new Error(`Can't resolve type parameter ${type.getSymbol()?.getName()} of property ${property.getName()}`);
        }
        const constraint = found.getConstraint();
        if (!constraint) {
            throw new Error(`No constriant on type parameter ${type.getSymbol()?.getName()} of property ${property.getName()}`);
        }
...
const getPropertiesWithTags = (properties: PropertySignature[]) => (...tagNames: string[]) => {
    return properties.filter(prop => prop.getJsDocs().filter(doc => (doc.getTags().some(t => tagNames.includes(t.getTagName())))).length > 0);
};

const getPropertiesWithoutTags = (properties: PropertySignature[]) => (...tagNames: string[]) => {
    return properties.filter(prop => prop.getJsDocs().filter(doc => (doc.getTags().some(t => tagNames.includes(t.getTagName())))).length === 0);
};
