...
sourceFiles.forEach((sourceFile) => {
    ...
    const callExpr = declaration.getDescendantsOfKind(SyntaxKind.CallExpression)[0];
    ...
    if (contextType) {
        ...
        sourceFile.addStatements(`function withContext<T>(ctx: unknown): asserts ctx is T { }`);
    }
    const contextName = contextType ? `<${suiteName}Context>` : '';
    declaration.remove();

    sourceFile.getDescendantsOfKind(ts.SyntaxKind.ExpressionStatement).forEach(
        (statement) => {
            ...
            if (expression.includes(`${suiteName}.before.each`)) {
                const arrow = statement.getExpression().getDescendantsOfKind(SyntaxKind.ArrowFunction)[0].getText();
                ...
                const callExpr = statement.getDescendantsOfKind(SyntaxKind.CallExpression)[0];
                const args = callExpr.getArguments();
                const testName = args[0].getText();
                const testFunction = args[1].getText();
                statement.remove();
                parent.insertStatements(idx, `test${contextName}(${testName}, ${testFunction});`);
    ...
    const expressions = sourceFile.getChildrenOfKind(SyntaxKind.ExpressionStatement);

    const describeStatement = sourceFile.addStatements(`describe${contextName}(${suiteDescription}, () => {});`)[0];
    const arrowFunc = describeStatement.getDescendantsOfKind(SyntaxKind.ArrowFunction)[0];
    if (arrowFunc) {
