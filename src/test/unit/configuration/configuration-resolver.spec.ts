...
const ConfigurationResolverSuite = suite<{
  importer: ISimpleResourceImporter;
  eventbus: Eventbus;
}>('ConfigurationResolver');

ConfigurationResolverSuite.before.each((context) => {
  context.importer = new MockImporter();
  context.eventbus = new MockEventbus() as Eventbus;
});

ConfigurationResolverSuite('should create', (context) => {
  // given
...
ConfigurationResolverSuite('should initialize event actions', (context) => {
  // given
  const { importer, eventbus } = context;
...
ConfigurationResolverSuite('should initialize initActions', (context) => {
  // give
  const { importer, eventbus } = context;
  const config = createTestConfig();
  config.initActions = [
...
ConfigurationResolverSuite('should initialize actions', (context) => {
  // given
  const { importer, eventbus } = context;
  const config = createTestConfig();
  config.actions = [
...
ConfigurationResolverSuite('should initialize timeline actions', (context) => {
  // give
  const { importer, eventbus } = context;
  const config = createTestConfig();
  config.timelines = [
...
ConfigurationResolverSuite('should resolve config: properties', (context) => {
  // given
  const { importer, eventbus } = context;
  const config = createTestConfig();
...
ConfigurationResolverSuite('should resolve json: properties', (context) => {
  // given
  const { importer, eventbus } = context;
  const config = createTestConfig();
  config.id = 'json:test';
  const resolver = new ConfigurationResolver(importer, eventbus);
  (importer as unknown as MockImporter).addEntry('test', { test: 'testValue' });
...
ConfigurationResolverSuite('should resolve template: properties', (context) => {
  // give
  const { importer, eventbus } = context;
  const config = createTestConfig();
...
ConfigurationResolverSuite.run();
