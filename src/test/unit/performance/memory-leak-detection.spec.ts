...
interface MemoryTestContext {
  eventbus: IEventbus;
  mockElement: any;
  originalOnPopstate: ((this: WindowEventHandlers, ev: PopStateEvent) => any) | null;
}
...
describe<MemoryTestContext>('Memory Leak Detection', () => {
  ...
  test<MemoryTestContext>(
    'LottieController attach/detach cycles should not leak memory',
    context => {
      ...
      context.eventbus.on = vi.fn((eventName: string, handler: (...args: any[]) => void) => {
        attachCount++;
  ...
  test<MemoryTestContext>(
    'LabelController attach/detach cycles should not leak memory',
    context => {
  ...
  test<MemoryTestContext>(
    'ProgressbarController attach/detach cycles should not leak memory',
    context => {
  ...
  test<MemoryTestContext>(
    'NavigationController attach/detach cycles should not leak memory',
    context => {
  ...
  test<MemoryTestContext>(
    'RoutingController attach/detach cycles should not leak memory',
    context => {
  ...
  test<MemoryTestContext>(
    'SubtitlesController attach/detach cycles should not leak memory',
    context => {
  ...
  test<MemoryTestContext>(
    'BaseController pattern should ensure consistent cleanup across all controllers',
    () => {
