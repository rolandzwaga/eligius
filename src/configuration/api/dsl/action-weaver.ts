...
type IOperationsOwner<T> = {
    [K in keyof Operations]: (operationData: Parameters<Operations[K]>[0]) => IOperationMixin<T>;
}
    ...
    constructor(
        ...
        const methods = Object.fromEntries(
            Object.keys(operations).map(name => [
                name,
                (function (this: OperationMixin, operationData: operations.TOperationData) {
                    return this.actionCreator[this.methodName](name as any, operationData);
                }).bind(this)
            ])
        );
...
export class TemplateWeaver {
    ...
    constructor(name: string, private factory: ConfigurationFactory) {
        this._actionCreator = this.factory.createAction(name);
    }

    weaveStartOperations() {
        return new OperationsWeaver(this._actionCreator, 'addStartOperation', this) as unknown as IOperationMixin<TemplateWeaver>;
    }

    weaveEndOperations() {
        return new OperationsWeaver(this._actionCreator, 'addEndOperation', this.factory) as unknown as IOperationMixin<ConfigurationFactory>;
    }

}

export class OperationsWeaver extends OperationMixin {
    constructor(actionCreator: EndableActionCreator, methodName: 'addStartOperation'|'addEndOperation', private _next: unknown){
