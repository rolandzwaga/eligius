...
import { EndableAction, TimelineAction } from '../action';
...
export class ConfigurationResolver2 implements IConfigurationResolver {
  ...
  process(
    ...
    processConfiguration(configuration, configuration, this.importer);
...
function processConfiguration(config: any, rootConfig: any, importer: IResourceImporter) {
  if (!isDefined(config)) {
    return;
  }
  ...
  if (Array.isArray(config)) {
    config.forEach((item) => {
      processConfiguration(item, rootConfig, importer);
    });
  } else {
    Object.keys(config).forEach((key) => {
      processConfigProperty(key, config, rootConfig, importer);
    });
  }
}

function processConfigProperty(key: string, config: any, rootConfig: any, importer: IResourceImporter) {
  ...
  if (typeof value === 'string') {
    ...
    processConfiguration(config[key], rootConfig, importer);
  }
}
