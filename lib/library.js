...
var ChronoTriggerEngine = /*#__PURE__*/function () {
  function ChronoTriggerEngine(configuration, eventbus, timelineProvider, languageManager) {
  ...
  _createClass(ChronoTriggerEngine, [{
    ...
    value: function _initializeTimelineProvider() {
      ...
      var timelineProviderSettings = this._configuration.timelineProviderSettings;

      if (!timelineProviderSettings) {
        return;
      }

      var playerContainer = (0, _jquery["default"])("#".concat(timelineProviderSettings.selector));

      if (playerContainer.length) {
        return new Promise(function (resolve) {
          _this._timelineProvider.init().then(function () {
            _this._executeActions(_this._configuration.initActions, 'start').then(function () {
              _this._timelineProvider.on(_timelineEventNames["default"].TIME, _this._onTimeHandler.bind(_this, Math.floor));

              _this._timelineProvider.on(_timelineEventNames["default"].SEEK, _this._onSeekHandler.bind(_this, Math.floor));

              resolve(_this._timelineProvider);
            });
          });
        });
      }
    ...
    value: function _handleRequestTimelineUri(index, position, previousVideoPosition) {
      ...
      this._cleanUpTimeline().then(function () {
        var timelineConfig = _this4._configuration.timelines[index];
        _this4._currentTimelineUri = timelineConfig.uri;
        _this4._timelineProvider.loop = timelineConfig.loop;

        if (!_this4._timelineProvider.loop && position > 0) {
          _this4._timelineProvider.once(_timelineEventNames["default"].FIRST_FRAME, function () {
            _this4._timelineProvider.pause();

            _this4._eventbus.broadcastForTopic(_timelineEventNames["default"].DURATION, _this4._timelineProvider.playerid, [_this4.getDuration()]);

            _this4._executeStartActions().then(function () {
              _this4._timelineProvider.seek(position);
        ...
        _this4._timelineProvider.playlistItem(index);
      });
    }
  }, {
    ...
    value: function _onTimeHandler(floor, event) {
      if (!isNaN(event.position)) {
        ...
        if (this._lastPosition !== pos) {
          ...
          this._eventbus.broadcastForTopic(_timelineEventNames["default"].POSITION_UPDATE, this._timelineProvider.playerid, [pos, this._timelineProvider.getDuration()]);
        }

        this._eventbus.broadcastForTopic(_timelineEventNames["default"].TIME_UPDATE, this._timelineProvider.playerid, [event.position, this._timelineProvider.getDuration()]);
      }
    }
  }, {
    key: "_onSeekHandler",
...
var EngineFactory = /*#__PURE__*/function () {
  ...
  _createClass(EngineFactory, [{
    ...
    value: function createEngine(configuration, resolver) {
      ...
      var timelineProvider = new timelineProviderClass(this._eventbus, configuration);

      _mousetrap["default"].bind('space', function (event) {
        event.preventDefault();

        _this2._eventbus.broadcastForTopic(_timelineEventNames["default"].PLAY_TOGGLE_REQUEST, timelineProvider.providerid);
      ...
      var chronoTriggerEngine = new engineClass(configuration, this._eventbus, timelineProvider, languageManager);
      return chronoTriggerEngine;
    }
  }]);
...
var MediaElementTimelineProvider = /*#__PURE__*/function () {
  ...
  _createClass(MediaElementTimelineProvider, [{
    ...
    value: function init(selector) {
      ...
      this.player = new _mediaelement["default"](document.querySelector('.mejs__player'), {
    ...
    value: function _addVideoElements(selector, urls) {
      ...
      var videoElm = ['<video class=\'mejs__player\'>'];
      urls.forEach(function (url) {
        videoElm.push("<source src='".concat(url, "' type='").concat(_this._extractFileType(url), "'/>"));
      });
      videoElm.push('</video>');
